unit Rasxod_mater_otgr_prixod_order_last_buy_for_bux;
...
type
  TFRasxod_mater_otgr_prixod_order_last_buy_for_bux = class(TForm)
    Edit1: TEdit;
    Edit2: TEdit;
    OraQuery1: TOraQuery;
...
var
  FRasxod_mater_otgr_prixod_order_last_buy_for_bux: TFRasxod_mater_otgr_prixod_order_last_buy_for_bux;
...
procedure TFRasxod_mater_otgr_prixod_order_last_buy_for_bux.FormShow(Sender: TObject);
begin
...
procedure TFRasxod_mater_otgr_prixod_order_last_buy_for_bux.Button2Click(Sender: TObject);
var
...
tx:=tx+'select distinct tronix.sele_max_id_prixod(tt.spravid,tt.godins) as obosn';

if CheckBox1.Checked=true then
tx:=tx+'select distinct tronix.sele_max_id_prixod(tt.spravid,'+godz+') as obosn';

tx:=tx+' from(';

tx:=tx+' select distinct (s.kod||substr(to_char( tn.date_ins, ''DD.MM.YYYY'' ),7,4)),s.kod kod,';
tx:=tx+' substr(to_char( tn.date_ins, ''DD.MM.YYYY'' ),7,4) godins,s.sprav_id spravid';
...
{*
  With OraQuery1 Do
     begin
        FieldByName('god').DisplayLAbel:='√Œƒ ';
        FieldByName('prix').DisplayLAbel:='œ–»’Œƒ ';
...
Sheet.Cells[nk,11].Value:='Ã¿“≈–»¿À€_œŒ—À≈ƒÕﬂﬂ_«¿ ”œ ¿';
...
procedure TFRasxod_mater_otgr_prixod_order_last_buy_for_bux.DateTimePicker1Change(Sender: TObject);
begin
dn:=FormatDateTime('yyyymmdd',DateTimePicker1.DateTime);
end;

procedure TFRasxod_mater_otgr_prixod_order_last_buy_for_bux.DateTimePicker2Change(Sender: TObject);
begin
dk:=FormatDateTime('yyyymmdd',DateTimePicker2.DateTime);
end;

procedure TFRasxod_mater_otgr_prixod_order_last_buy_for_bux.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     OraQuery1.Close;
end;

procedure TFRasxod_mater_otgr_prixod_order_last_buy_for_bux.TxtgodzChange(
  Sender: TObject);
begin
godz:=TxtGodz.Text;
end;

procedure TFRasxod_mater_otgr_prixod_order_last_buy_for_bux.CheckBox1Click(
  Sender: TObject);
begin
if CheckBox1.Checked=false then
