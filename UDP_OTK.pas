...
procedure TFUDP_OTK.SelectData;
var tx: string;
begin
tx:=' ';
...
tx:=tx+' from dic_concept co where co.shortname like ('''+edit2.Text+'У%%'')';
tx:=tx+' )ll';
...
procedure TFUDP_OTK.FormShow(Sender: TObject);
...
txx:=txx+' and pii.type_prikaz_type_prikaz_id=tpp.type_prikaz_id and tpp.kod=1 and pii.dep_dep_id=de.dep_id';
 ...
 cexuser:=OraQuery3.FieldByName('nomer').asString;
  ...
  begin
   FUDP_OTK.CheckBox1.Enabled:=false;
   FUDP_OTK.CheckBox2.Enabled:=false;
   FUDP_OTK.CheckBox3.Enabled:=false;
   FUDP_OTK.Button2.Enabled:=false;
   FUDP_OTK.RadioGroup1.Enabled:=false;
   FUDP_OTK.DBGrideh1.Columns[6].ReadOnly:=true;
   end;
...
procedure TFUDP_OTK.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  OraQuery1.Close;
end;

procedure TFUDP_OTK.DBGridEh1DblClick(Sender: TObject);
...
procedure TFUDP_OTK.Button2Click(Sender: TObject);
var tx1: string;
begin
...
procedure TFUDP_OTK.Button1Click(Sender: TObject);
var
