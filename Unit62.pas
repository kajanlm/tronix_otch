...
tx:=' ';
tx:='select tt.podr podr,tt.priznak priznak,tt.zak zak,substr(tt.kodprof,1,5) kodprof,tt.prof prof,sum(tt.trudoem) trudoem';
tx:=tx+' from (';
// tt
tx:=tx+' select de.nomer podr,''Ð'' priznak, pr.zavn zak, kv.kod_prof kodprof, kv.name prof,tm.trudoem trudoem ';
...
tx:=tx+' select de.nomer podr,''Ï'' priznak,pr.zavn zak,kv.kod_prof kodprof,kv.name prof,tm.trudoem trudoem';
...
tx:=tx+' and de.dep_id=tn.dep_dep_id_to';
tx:=tx+' ) tt';
tx:=tx+' group by tt.podr,tt.priznak, tt.zak, tt.kodprof,tt.prof';
tx:=tx+' order by tt.podr,tt.priznak,tt.zak,tt.kodprof';
...
procedure TForm62.DateTimePicker1Change(Sender: TObject);
begin
dn:=FormatDateTime('yyyymmdd',DateTimePicker1.DateTime);
end;

procedure TForm62.DateTimePicker2Change(Sender: TObject);
begin
dk:=FormatDateTime('yyyymmdd',DateTimePicker2.DateTime);
end;

procedure TForm62.DBGridEh1DrawColumnCell(Sender: TObject;
