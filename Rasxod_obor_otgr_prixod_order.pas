...
tx:=' ';
tx:='select tt.proe as proe,tt.godins as godins,tt.mesins as mesins,tt.prixo as prixo,tt.schfp as schfp,tt.tnp as tnp,tt.post as post,tt.inn as inn';
tx:=tx+' from(';
tx:=tx+' select distinct(t1.prix||t1.schf||t1.nacl) prix,t1.mesins mesins,t1.godins godins,t1.prix prixo,t1.schf schfp,t1.nacl tnp,t1.post post,t1.inn inn,t1.proe proe';
tx:=tx+' from(';
...
tx:=tx+' substr(to_char( tp.date_ins, ''DD.MM.YYYY'' ),4,2) mesins,substr(to_char( tp.date_ins, ''DD.MM.YYYY'' ),7,4) godins ,fi.ident post,fi.kod_ni inn,pr.name proe';
...
tx:=tx+' and tronix.select_mat(s1.tree_tree_id,''02'')=1';
