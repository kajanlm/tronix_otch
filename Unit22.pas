...
tx:=tx+' l.tnnomer, l.tndatedok, l.tndateins, l.ednamek, l.koluchet , l.ednamek1, l.koluchet1, l.tnn ,l.tndatedok2 ';
tx:=tx+'from ';
tx:=tx+'( ';
tx:=tx+' select z.zavn, do.ident, ss.poz, ss.podpoz, replace(tronix_sp_name(sp.sprav_id), chr(10), '' '') rp,';
tx:=tx+'  sp.kod,ed.namek, ss.kol,     ';
tx:=tx+'   '' '' tnnomer,'' '' tndatedok, '' '' tndateins,'' '' tndatedok2,'' '' ednamek, 0 koluchet,    ';
...
tx:=tx+'       ll.tndateins tndateins,  ll.tndatedok2 ,    ';
...
tx:=tx+'        to_char( tn.date_ins, ''DD.MM.YYYY'' ), to_char( tn.user_date2, ''DD.MM.YYYY'' ), ';
