...
//ShowMEssage(idpro);
if idpro='' then
ShowMessage('Внимание! Необходимо выбрать проекты')
else
begin
OraQuery1.Close;
...
//ShowMEssage(txx);
  With OraQuery1 Do
     begin
        FieldByName('zavn').DisplayLAbel:='ЗАВ.№ ';
        FieldByName('zak').DisplayLAbel:='ЗАКАЗ';
        FieldByName('proekt').DisplayLAbel:='ПРОЕКТ ';
        FieldByName('vidnorm').DisplayLAbel:='ВИД НОРМ ';
        FieldByName('trud').DisplayLAbel:='ТР-ТЬ ';
     end;
