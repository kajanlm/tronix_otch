...
var
  FNaryd_Close_PDO: TFNaryd_Close_PDO;
  dn: string;
  dk: string;
implementation
...
procedure TFNaryd_Close_PDO.FormShow(Sender: TObject);
begin
...
procedure TFNaryd_Close_PDO.FormClose(Sender: TObject; var Action: TCloseAction);
begin
     OraQuery1.Close;
end;

procedure TFNaryd_Close_PDO.Button1Click(Sender: TObject);
var
...
procedure TFNaryd_Close_PDO.Button2Click(Sender: TObject);
var tx:string;
begin
...
tx:=tx+' and tn.CADRY_CADRY_ID_NAR=pi.CADRY_CADRY_ID(+) and pi.type_prikaz_type_prikaz_id=tp.type_prikaz_id and tp.kod=1';
tx:=tx+' and pi.CADRY_CADRY_ID=na.CADRY_CADRY_ID(+) and tm.trudoem is not null and tn.date_anul_nar is null';

tx:=tx+' and nvl(nordsy.go_in_tk(tx.TEXkompl_TEXKOMPL_ID,''œ”≈'',''TYPE''),tx.TEXkompl_TEXKOMPL_ID)=tk.texkompl_id and nordsy.uzak_tx(tx.TEXkompl_TEXKOMPL_ID)=z.nn(+)';
...
tx:=tx+' and tn.uzak_uzak_id=zk.nn(+)';
tx:=tx+' and tm.trudoem is not null and tn.date_anul_nar is null and nvl(nordsy.go_in_tk(tx.TEXkompl_TEXKOMPL_ID,''œ”≈'',''TYPE''),tx.TEXkompl_TEXKOMPL_ID)=tk.texkompl_id';
...
procedure TFNaryd_Close_PDO.DateTimePicker1Change(Sender: TObject);
begin
dn:=FormatDateTime('yyyymmdd',DateTimePicker1.DateTime);
end;

procedure TFNaryd_Close_PDO.DateTimePicker2Change(Sender: TObject);
begin
dk:=FormatDateTime('yyyymmdd',DateTimePicker2.DateTime);
end;
